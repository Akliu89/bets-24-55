...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-ede1eaae";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-ede1eaae">'+'<span class="tile-face-ede1eaae tile-front-ede1eaae"></span>'+'<span class="tile-face-ede1eaae tile-back-ede1eaae"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ede1eaae")){return;}
var back=tile.querySelector(".tile-back-ede1eaae");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-ede1eaae");if(isMine){tile.classList.add("boom-ede1eaae");}
if(dim){tile.classList.add("dimmed-ede1eaae");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-ede1eaae")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-ede1eaae");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ede1eaae")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-ede1eaae");void panelEl.offsetWidth;panelEl.classList.add("jolt-ede1eaae");}
endRound();}
